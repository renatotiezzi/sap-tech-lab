...
|--------|------|------|-------------|
| `ZI_Q2C_ARQ_MGR` | CDS Root View Entity | `ZI_Q2C_ARQ_MGR.ddls.txt` | Root BO. Maps `ztbq2c_arq_mgr`. Defines composition to `_Log`. |
| `ZI_Q2C_LOG_MGR` | CDS View Entity (child) | `ZI_Q2C_LOG_MGR.ddls.txt` | Child BO. Maps `ztbq2c_log_mgr`. Parent association `_Arq`. |
| `ZI_Q2C_ARQ_MGR` | BDEF (managed) | `ZI_Q2C_ARQ_MGR.bdef.txt` | Defines `create/update/delete`, actions `Reprocess`/`Cancel`, determination `set_admin_fields`, lock/auth master. |
| `ZBP_I_Q2C_ARQ_MGR` | Behavior Pool (global class) | `ZBP_I_Q2C_ARQ_MGR.clas.txt` | Handler class. Abstract final. FOR BEHAVIOR OF `zi_q2c_arq_mgr`. |
| `ZBP_I_Q2C_ARQ_MGR` CCIMP | Local class in behavior pool | `ZBP_I_Q2C_ARQ_MGR.clas.locals_imp.txt` | Contains: `lhc_arq_mgr` (actions + determination), `lhc_log_mgr` (determination), `upsert_log` (private helper). |
| `ZCL_Q2C_CPI_CALLER` | Service class | `CR51_APP/ZCL_Q2C_CPI_CALLER.clas.txt` | HTTP caller for CPI integration. Called directly from behavior pool actions. |

---

### ADMIN SERVICE — ZC Layer
...
| Object | Type | File | Description |
|--------|------|------|-------------|
| `ZC_Q2C_ARQ_MGR` | CDS Root Projection View | `ZC_Q2C_ARQ_MGR.ddls.txt` | Projection of `ZI_Q2C_ARQ_MGR`. `provider contract transactional_query`. Redirects `_Log` to `ZC_Q2C_LOG_MGR`. |
| `ZC_Q2C_LOG_MGR` | CDS Projection View (child) | `ZC_Q2C_LOG_MGR.ddls.txt` | Projection of `ZI_Q2C_LOG_MGR`. No `provider contract`. Redirects `_Arq` to `ZC_Q2C_ARQ_MGR`. |
| `ZC_Q2C_ARQ_MGR` | BDEF (projection) | `ZC_Q2C_ARQ_MGR.bdef.txt` | Exposes: `use create; use update; use delete; use association _Log { create; }`. No actions. |
| `ZSD_Q2C_MGR` | Service Definition | `ZSD_Q2C_MGR.srvd.txt` | Exposes `ZC_Q2C_ARQ_MGR as ArqMgr` + `ZC_Q2C_LOG_MGR as LogMgr`. |
| `ZSB_Q2C_MGR` | Service Binding (OData V4 UI) | `ZSB_Q2C_MGR.srvb.txt` *(reference only)* | Must be created manually in ADT. Requires Publish. |

---

### APP SERVICE — ZC_APP Layer
...
### AUXILIARY — Standalone Classes

| Object | Type | File | Description |
|--------|------|------|-------------|
| `ZCL_Q2C_REPROCESS_ACTION` | Service class | `CR51_APP/ZCL_Q2C_REPROCESS_ACTION.clas.txt` | Standalone reprocessamento logic for use **outside RAP** (jobs, programs). Not used by the Fiori app. |
