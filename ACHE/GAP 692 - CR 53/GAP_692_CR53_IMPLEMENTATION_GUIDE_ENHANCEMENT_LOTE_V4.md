...
| Legado (`YYPCL_*`) | CDS View nova | SQL View / tabela física | Chave | Campos |
|---|---|---|---|---|
| `YYPCL_AGRUPMTART` | `/ACHE/I_MMCDS_1001` — "Agrupamento x tipo de material" | `/ache/mmt100` | `MTART` | `MTART` (era chave), `GRPMA` (era `ZZGRPMA`) |
| `YYPCL_AGRUPLOTES` | `/ACHE/I_MMCDS_1002` — "Agrupamento x qtd caract. x quebra mensal" | `/ache/mmt101` | `GRPMA` | `GRPMA` (chave, era `ZZGRPMA`), `SEQLOTE` (era `ZZSEQLOTE`), `SEQMENSAL` (era `ZZSEQMENSAL`) |
| `YYPCL_CNTLOTEMES` | `/ACHE/I_MMCDS_1003` — "Controle de Lotes x quebra mensal" | `/ache/mmt103` | `GJAHR` + `MONAT` + `GRPMA` | `SEQ05`/`SEQ06`/`SEQ08` (eram `ZZSEQ05`/`ZZSEQ06`/`ZZSEQ08`) |
| `YYPCL_CNTLOTE` | `/ACHE/I_MMCDS_1004` — "Controle de Lotes x quebra anual" | `/ache/mmt104` | `GJAHR` + `GRPMA` | `SEQ05`/`SEQ06`/`SEQ08` (idem) |

> **O prefixo `ZZ` sumiu.** Fazia sentido no legado porque eram campos de anexo (append) sobre estrutura standard; nas tabelas `/ACHE/MMT1xx`, 100% customizadas, não é mais necessário. Já ajustei todo o código deste guia (seções 3, 5 e 7) para os nomes sem `ZZ`.
>
...
Por Clean Core (EF seção 1.8.1), sua função central deve fazer **leitura via CDS View** (`/ACHE/I_MMCDS_1001` a `1004`), não diretamente nas tabelas físicas `/ache/mmt1xx`. Gravação (incremento de contador) é feita direto na tabela física — pergunte ao dev do RAP App se existe alguma ação/BO exposta pra isso antes de gravar direto na tabela, para não pular alguma validação do lado dele.

### 2.2 Por que três campos de contador (`SEQ05`/`SEQ06`/`SEQ08`)

A regra de range (EF seção 2.5) prevê sequenciais de 5, 6 ou 8 dígitos dependendo do grupo. Cada grupo usa **um** desses campos (definido por `SEQLOTE` em `/ACHE/I_MMCDS_1002`); os outros dois ficam em branco/zero para aquele grupo. Sua função central deve ler/incrementar o campo certo com base em `SEQLOTE`, não os três — exatamente como a FM legada fazia com `CASE it_yypcl_agruplotes-zzseqlote`.
...
> Isso também simplifica a chamada a partir da BAdI AFTER (seção 7.4): passe `batch_allocation-material_type` diretamente como `i_mtart`, sem precisar de um `SELECT` intermediário em `I_Material`.
...
```abap
  DATA: lv_grpma      TYPE grpma,              " confirmar elemento de dados exato com o dev do RAP App
        ls_agruplotes TYPE /ache/i_mmcds_1002,
        ls_cntlotemes TYPE /ache/i_mmcds_1003,
        ls_cntlote    TYPE /ache/i_mmcds_1004,
        lv_gjahr      TYPE gjahr,
        lv_monat      TYPE monat,
...
* ------------------------------------------------------------------
  SELECT SINGLE grpma FROM /ache/i_mmcds_1001
    INTO lv_grpma
    WHERE mtart = i_mtart.
  IF sy-subrc <> 0.
    e_status = 'E'.
...
* ------------------------------------------------------------------
  IF ls_agruplotes-seqmensal IS NOT INITIAL.
    ...
    SELECT SINGLE * FROM /ache/i_mmcds_1003
      INTO ls_cntlotemes
      WHERE grpma = lv_grpma AND gjahr = lv_gjahr AND monat = lv_monat.
    IF sy-subrc <> 0.
    ...
    PERFORM f_montar_lote_mensal USING ls_agruplotes-seqlote lv_gjahr lv_monat
  ...
  ELSE.
    ...
    SELECT SINGLE * FROM /ache/i_mmcds_1004
      INTO ls_cntlote
      WHERE grpma = lv_grpma AND gjahr = lv_gjahr.
    IF sy-subrc <> 0.
    ...
    PERFORM f_montar_lote_anual USING ls_agruplotes-seqlote lv_gjahr
...
* ------------------------------------------------------------------
  ...
  WHILE lv_existe = abap_true.
    ...
    DATA(lv_seq_atual) = COND i( WHEN ls_agruplotes-seqmensal IS NOT INITIAL THEN
                                   COND #( WHEN ls_agruplotes-seqlote = '5' THEN ls_cntlotemes-seq05
                                           WHEN ls_agruplotes-seqlote = '6' THEN ls_cntlotemes-seq06
                                           WHEN ls_agruplotes-seqlote = '8' THEN ls_cntlotemes-seq08 )
                                 ELSE
                                   COND #( WHEN ls_agruplotes-seqlote = '5' THEN ls_cntlote-seq05
                                           WHEN ls_agruplotes-seqlote = '6' THEN ls_cntlote-seq06
                                           WHEN ls_agruplotes-seqlote = '8' THEN ls_cntlote-seq08 ) ).
...
* ------------------------------------------------------------------
  IF ls_agruplotes-seqmensal IS NOT INITIAL.
    MODIFY /ache/mmt103 FROM ls_cntlotemes.
    CALL FUNCTION '/ACHE/DEQUEUE_EMMLOTEMES'
      EXPORTING mandt = sy-mandt grpma = lv_grpma gjahr = lv_gjahr monat = lv_monat.
  ELSE.
    MODIFY /ache/mmt104 FROM ls_cntlote.
    CALL FUNCTION '/ACHE/DEQUEUE_EMMLOTEANO'
      EXPORTING mandt = sy-mandt grpma = lv_grpma gjahr = lv_gjahr.
  ENDIF.
...
| `SEQMENSAL` | `SEQLOTE` | Fórmula (legado, preservada na V4) | Campo de contador |
|---|---|---|---|
| Mensal (`X`) | `5` | `YY` + `SEQ05` | `/ache/i_mmcds_1003-seq05` |
| Mensal (`X`) | `6` | `YY` + `MM` + `SEQ06` | `/ache/i_mmcds_1003-seq06` |
| Mensal (`X`) | `8` | `YY` + `SEQ08` | `/ache/i_mmcds_1003-seq08` |
| Anual (vazio) | `5` | `YY` + `SEQ05` | `/ache/i_mmcds_1004-seq05` |
| Anual (vazio) | `6` | `YY` + `SEQ05`* | `/ache/i_mmcds_1004-seq06` |
| Anual (vazio) | `8` | `YY` + `SEQ08` | `/ache/i_mmcds_1004-seq08` |
...
```abap
FORM f_montar_lote_mensal USING    u_seqlote     TYPE seqlote
                                    u_gjahr       TYPE gjahr
                                    u_monat       TYPE monat
  ...
  CASE u_seqlote.
    WHEN '5'.
      c_cntlotemes-seq05 = c_cntlotemes-seq05 + 1.
      CONCATENATE u_gjahr+2(2) c_cntlotemes-seq05 INTO c_lote.
    WHEN '6'.
      c_cntlotemes-seq06 = c_cntlotemes-seq06 + 1.
      CONCATENATE u_gjahr+2(2) u_monat c_cntlotemes-seq06 INTO c_lote.
    WHEN '8'.
      c_cntlotemes-seq08 = c_cntlotemes-seq08 + 1.
      CONCATENATE u_gjahr+2(2) c_cntlotemes-seq08 INTO c_lote.
  ENDCASE.

ENDFORM.

FORM f_montar_lote_anual USING    u_seqlote  TYPE seqlote
                                   u_gjahr    TYPE gjahr
  ...
  CASE u_seqlote.
    WHEN '5'.
      c_cntlote-seq05 = c_cntlote-seq05 + 1.
      CONCATENATE u_gjahr+2(2) c_cntlote-seq05 INTO c_lote.
    WHEN '6'.
      c_cntlote-seq06 = c_cntlote-seq06 + 1.
      CONCATENATE u_gjahr+2(2) c_cntlote-seq06 INTO c_lote.   " sem mês, fiel ao legado
    WHEN '8'.
      c_cntlote-seq08 = c_cntlote-seq08 + 1.
      CONCATENATE u_gjahr+2(2) c_cntlote-seq08 INTO c_lote.
  ENDCASE.
...
**O que isso muda na prática**: este método não serve para "escrever o lote". Ele serve para **desviar o SAP standard do caminho dele**, avisando que a numeração interna (e a de Custom BO) não deve rodar para este material — porque quem vai determinar o lote é a sua lógica Aché, na BAdI **AFTER** (seção 7.4).
...
METHOD if_lobm_before_batch_nmbr_int~before_number_assignment.
  ...
  DATA: lv_grpma TYPE grpma.   " confirmar elemento de dados com o dev do RAP App
...
* ------------------------------------------------------------------
  SELECT SINGLE grpma FROM /ache/i_mmcds_1001
    INTO lv_grpma
    WHERE mtart = batch_allocation-material_type.       " nome do componente a confirmar via SE24
...
* definitivo é calculado na BAdI AFTER (seção 7.4), chamando a função
* central.
...
4. Adicione as duas BAdI Definitions do spot à mesma implementação: `LOBM_BEFORE_BATCH_NUMBER_INT` e a BAdI **AFTER** (nome exato a confirmar — seção 7.4). A SE19 normalmente permite agrupar BAdIs do mesmo enhancement spot numa única implementação, com uma classe implementando as duas interfaces.
5. Nome da classe implementadora: `/ACHE/CLMM_LOTE_BADI` (WORKBOOK SAPHIRA §5 — `/ACHE/CLXX_<Descrição>`).
6. Grave, ative.
7. Abra a classe gerada pela **SE24** e implemente os dois métodos (7.2 e 7.4).

### 7.4 Localizar e implementar a BAdI AFTER — onde o lote é gravado
...
O esqueleto abaixo é uma expectativa razoável por simetria com o BEFORE (importa/altera `BATCH_ALLOCATION`, permite escrever no campo de lote dentro dela, e usa a mesma exceção `CX_BLE_RUNTIME_ERROR`) — **trate como rascunho até confirmar na SE24**:
...
### 7.5 Regra de preservação de lote manual — onde ela vive de verdade agora
...
3. Substitua (ou insira) a chamada para `/ACHE/MMF_LOTE_DETERMINAR`, com a mesma assinatura usada na BAdI AFTER (seção 7.4 — note que o parâmetro chave agora é `I_MTART`, direto, sem depender de `MATNR`), passando a data-base correta do fluxo do coletor (a EF não especifica qual campo de data o coletor usa — confirme com quem mantém o GAP334/391).
...
- [ ] Nome exato da BAdI/interface/método **AFTER** confirmado via SE18/SE24 (o guia assume `LOBM_AFTER_BATCH_NUMBER_INT` / `IF_LOBM_AFTER_BATCH_NMBR_INT` por simetria, não confirmado em sistema — seção 7.4)
- [ ] Construtor de `CX_BLE_RUNTIME_ERROR` confirmado via SE24 — como propagar a mensagem da classe `/ACHE/MM_LOTE` através dessa exceção (seção 7.4)
