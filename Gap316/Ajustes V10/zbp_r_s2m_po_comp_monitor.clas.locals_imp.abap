CLASS lhc_zr_s2m_materiais_compative DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    " V10 - RTIEZZI - Action retorna a instancia para atualizar estado local sem refresh de object page
    METHODS remarcar FOR MODIFY IMPORTING keys FOR ACTION zr_s2m_materiais_compativeis~remarcar RESULT result.

    " V10 - RTIEZZI - Desabilita o botao Remarcar para a linha ativa na reserva
    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR zr_s2m_materiais_compativeis RESULT result.
...
CLASS lhc_zr_s2m_materiais_compative IMPLEMENTATION.
  ...
  " V10 - RTIEZZI - INICIO - Desabilita Remarcar para a linha que ja e o material/lote ativo na reserva
  METHOD get_instance_features.
    ...
    LOOP AT lt_data ASSIGNING FIELD-SYMBOL(<fs_data>).
      ...
      SELECT SINGLE Product, Batch          "#EC CI_NOORDER "#EC WARNOK
        FROM I_ReservationDocumentItem
       WHERE Reservation     EQ @<fs_data>-reservation
         AND ReservationItem EQ @<fs_data>-reservationitem
        INTO @DATA(ls_ativo).

      APPEND VALUE #(
        %key = <fs_data>-%key
        %features-%action-remarcar = COND #(
          WHEN <fs_data>-material = ls_ativo-Product AND <fs_data>-charg = ls_ativo-Batch
  ...
  ENDMETHOD.
  " V10 - RTIEZZI - FIM - Desabilita Remarcar para a linha que ja e o material/lote ativo na reserva
     ...
     rspos = ls_po_comp_monitor-reservationitem )
      ...
      IF lt_po_comp_monitor IS NOT INITIAL.
        ...
        ELSE.
          ...
          " V10 - RTIEZZI - Retorna a instancia atualizada para refletir estado da linha apos sucesso
          READ ENTITIES OF zr_s2m_po_comp_monitor IN LOCAL MODE
            ENTITY zr_s2m_materiais_compativeis
            ALL FIELDS
          ...
          " V10 - RTIEZZI - Mensagem funcional de sucesso apos remarcacao
          APPEND VALUE #( %key = ls_material_comp-%key
