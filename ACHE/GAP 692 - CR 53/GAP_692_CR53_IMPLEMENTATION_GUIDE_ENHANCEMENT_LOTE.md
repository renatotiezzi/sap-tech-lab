...
BAdI: LOBM_BEFORE_BATCH_NUMBER_INT / BEFORE_NUMBER_ASSIGNMENT  ─┐
                                                                  ├──► FUNCTION ZMM_LOTE_DETERMINAR_ACHE (lógica central)
...
1. Abra **SE11**.
2. Selecione **Objeto de bloqueio** (Lock Object) → nome sugerido `EZMM_LOTEACHE` (confirme convenção de nomenclatura do cliente).
...
5. Crie um segundo objeto de bloqueio `EZMM_LOTEACHE_ANO` sobre `/ACHE/CNTLOTE`, campos `MANDT`, `ZZGRPMA`, `GJAHR` (bloqueio anual — conforme EF).
6. Ative. O SAP gera automaticamente as function modules `ENQUEUE_EZMM_LOTEACHE` / `DEQUEUE_EZMM_LOTEACHE` (e o par `_ANO`).
...
## 4. Passo 4 — Criar a classe de mensagens ZLOT

1. Abra **SE91**.
2. Classe de mensagens: `ZLOT` (ou padrão de nomenclatura do cliente, ex. `Z_MM_LOTE`).
3. Cadastre as mensagens conforme a EF (seção 2.6):
...
1. Abra **SE37** → crie um novo grupo de funções (ex. `ZMM_LOTE_ACHE`) se ainda não existir um adequado.
2. Crie a function module `ZMM_LOTE_DETERMINAR_ACHE` (confirme nome com o padrão do cliente; pode ser que o namespace `/ACHE/` também se aplique a objetos de programa — confirme).

### 5.1 Assinatura sugerida

```abap
FUNCTION zmm_lote_determinar_ache.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
*"     REFERENCE(IV_MATNR) TYPE  MATNR
*"     REFERENCE(IV_WERKS) TYPE  WERKS_D OPTIONAL
*"     REFERENCE(IV_DATA_BASE) TYPE  SY-DATUM
*"     REFERENCE(IV_LOTE_INFORMADO) TYPE  CHARG_D OPTIONAL
*"  EXPORTING
*"     REFERENCE(EV_LOTE) TYPE  CHARG_D
*"     REFERENCE(EV_STATUS) TYPE  CHAR1 " 'S' sucesso, 'N' não aplicável (lote já informado), 'E' erro
...
```abap
  ...
  CLEAR: ev_lote, ev_status, ev_msg.
...
* ------------------------------------------------------------------
  IF iv_lote_informado IS NOT INITIAL.
    ev_lote   = iv_lote_informado.
    ev_status = 'N'.
    MESSAGE i007(zlot) INTO ev_msg. "log informativo, não bloqueante
    RETURN.
  ENDIF.
...
* ------------------------------------------------------------------
  ...
  " se o tipo de material já vier de fora, prefira receber IV_MTART
...
* ------------------------------------------------------------------
  ...
  IF sy-subrc <> 0.
    ev_status = 'E'.
    MESSAGE e001(zlot) WITH lv_mtart INTO ev_msg.
...
* ------------------------------------------------------------------
  ...
  IF sy-subrc <> 0.
    ev_status = 'E'.
    MESSAGE e002(zlot) WITH lv_zzgrpma INTO ev_msg.
    RAISE sem_estrutura.
  ENDIF.

  lv_gjahr = iv_data_base(4).
  lv_monat = iv_data_base+4(2).

* ------------------------------------------------------------------
* Bloquear + ler contador (mensal ou anual, conforme estrutura)
* ------------------------------------------------------------------
  IF ls_agruplotes-zzseqmensal = abap_true.
    CALL FUNCTION 'ENQUEUE_EZMM_LOTEACHE'
      EXPORTING
        mandt   = sy-mandt
        zzgrpma = lv_zzgrpma
        gjahr   = lv_gjahr
  ...
  ELSE.
    ...
    PERFORM ler_contador_anual USING lv_zzgrpma lv_gjahr
                               CHANGING lv_seq lv_seq_limite.
    IF sy-subrc <> 0.
      CALL FUNCTION 'DEQUEUE_EZMM_LOTEACHE_ANO'
        EXPORTING mandt = sy-mandt zzgrpma = lv_zzgrpma gjahr = lv_gjahr.
      ev_status = 'E'.
      MESSAGE e003(zlot) WITH lv_zzgrpma lv_gjahr '' INTO ev_msg.
...
* ------------------------------------------------------------------
  ...
  IF lv_seq > lv_seq_limite.
    PERFORM liberar_lock USING ls_agruplotes-zzseqmensal lv_zzgrpma lv_gjahr lv_monat.
    ev_status = 'E'.
    MESSAGE e005(zlot) WITH lv_zzgrpma lv_gjahr INTO ev_msg.
...
* ------------------------------------------------------------------
  PERFORM montar_lote USING ls_agruplotes lv_gjahr lv_monat lv_seq
                       CHANGING lv_lote.
...
* ------------------------------------------------------------------
  WHILE lote_existe( lv_lote ) = abap_true.
    lv_seq = lv_seq + 1.
    IF lv_seq > lv_seq_limite.
      PERFORM liberar_lock USING ls_agruplotes-zzseqmensal lv_zzgrpma lv_gjahr lv_monat.
      ev_status = 'E'.
    ...
    PERFORM montar_lote USING ls_agruplotes lv_gjahr lv_monat lv_seq
                         CHANGING lv_lote.
  ENDWHILE.
  " lote_existe(): validar em I_BatchPlant / I_BatchCrossPlant (sucessores
...
* ------------------------------------------------------------------
  PERFORM atualizar_contador USING ls_agruplotes-zzseqmensal
...
> **Não faça `COMMIT WORK` dentro desta função nem dentro do `PERFORM atualizar_contador`.** A EF (seção 1.5) é explícita: o enhancement deve respeitar a LUW do processo SAP que o chamou (MIGO, ordem de processo, coletor). Um commit próprio aqui quebraria o tratamento de erro do processo standard e poderia liberar o lock antes da hora.
...
3. Nome da implementação: ex. `ZIM_LOBM_LOTE_ACHE`.
4. BAdI Definition: `LOBM_BEFORE_BATCH_NUMBER_INT`.
5. Nome da classe implementadora sugerido: `ZCL_IM_LOBM_BATCH_ACHE` (o SE19 pode gerar automaticamente — confirme convenção do cliente).
6. Grave, ative.
...
* barato e deixa a intenção explícita neste ponto de entrada.)
  ...
  CALL FUNCTION 'ZMM_LOTE_DETERMINAR_ACHE'
    EXPORTING
      iv_matnr          = iv_matnr
      iv_werks          = iv_werks
      iv_data_base      = sy-datum   " confirmar origem correta da data-base
  ...
  IF sy-subrc <> 0.
    ...
    MESSAGE ID 'ZLOT' TYPE 'E' NUMBER sy-msgno
...
3. Substitua (ou insira) a chamada para `ZMM_LOTE_DETERMINAR_ACHE`, com a mesma assinatura usada no BAdI (seção 7.3), passando a data-base correta do fluxo do coletor (a EF não especifica qual campo de data o coletor usa — confirme com quem mantém o GAP334/391).
4. Trate os códigos de retorno (`EV_STATUS`, exceções) da mesma forma que no BAdI: erro bloqueante impede a gravação da tela do coletor; sucesso preenche o campo de lote.
...
O tamanho aplicável por grupo vem de `ZZSEQLOTE` em `/ACHE/AGRUPLOTES` (a confirmar nome final da tabela — seção 2 deste guia). Ao atingir o limite, bloqueie a geração e emita `ZLOT005` — não tente "dar a volta" (wrap-around) para 1, a EF não prevê isso e criaria duplicidade certa com lotes já emitidos no início do range.
...
|---|---|---|
| CTN01 | Tipo de material sem parametrização em `AGRUPMTART` | `ZLOT001`, bloqueante |
| CTN02 | Grupo sem estrutura em `AGRUPLOTES` | `ZLOT002`, bloqueante |
| CTN03 | Contador mensal/anual inexistente para o período | `ZLOT003`, bloqueante (ver decisão de arquitetura, seção 6) |
| CTN04 | Lock do contador indisponível (concorrência — dois processos simultâneos) | `ZLOT004`, bloqueante, sem duplicidade gerada por nenhum dos dois processos |
| CTN05 | Range esgotado (sequencial no limite) | `ZLOT005`, bloqueante |
...
- [ ] Objetos de bloqueio criados e testados sob concorrência real (CTN04)
- [ ] Mensagens `ZLOT001`–`ZLOT007` criadas na SE91 com os textos exatos da EF
